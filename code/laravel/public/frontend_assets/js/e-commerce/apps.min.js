...
var handleHeaderNavigationState = function () {
    $(window).on("scroll load", function () {
        $(window).scrollTop() > 20 ? $("#header").addClass("navbar-sm") : $("#header").removeClass("navbar-sm")
    })
},handleHeaderFixedTop = function () {
    0 !== $('#header[data-fixed-top="true"]').length && $(window).on("scroll", function () {
        var e = $(window).width() >= 1920 ? 48 : 40, t = $(window).width() >= 1920 ? "100px" : "76px";
        $(window).scrollTop() >= e ? ($("body").css("padding-top", t), $("#header").addClass("header-fixed")) : ($("#header").removeClass("header-fixed"), $("body").css("padding-top", "0"))
    })
    ...
    if (Cookies && Cookies.get("theme")) {
        0 !== $(".theme-list").length && ($(".theme-list [data-theme]").closest("li").removeClass("active"), $('.theme-list [data-theme="' + Cookies.get("theme") + '"]').closest("li").addClass("active"));
        var e = $('[data-theme="' + Cookies.get("theme") + '"]').attr("data-theme-file");
    ...
    $(document).on("click", '[data-click="set-payment"]', function (e) {
        e.preventDefault();
        var t = $(this).closest("li"), a = $(this).attr("data-value");
    ...
    $(document).on("click", '[data-click="increase-qty"]', function (e) {
        e.preventDefault();
        var t = $(this).attr("data-target"), a = parseInt($(t).val()) + 1;
        $(t).val(a)
    ...
    $(document).on("click", '[data-click="show-main-image"]', function (e) {
        e.preventDefault();
        var t = '<img src="' + $(this).attr("data-url") + '" />', a = $(this).closest("li");
        $('[data-id="main-image"]').html(t), $(a).addClass("active"), $('[data-click="show-main-image"]').closest("li").not(a).removeClass("active")
    })
}, App = function () {
    "use strict";
    return {
        init: function () {
            handleHeaderNavigationState(),handleHeaderFixedTop(), handlePageContainerShow(), handlePaceLoadingPlugins(), handleTooltipActivation(), handleThemePanelExpand(), handleThemePageControl(), handlePaymentTypeSelection(), handleQtyControl(), handleProductImage()
        }
    }
}();